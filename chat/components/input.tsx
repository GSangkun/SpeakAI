...
"""`
    ...
    if (!response.ok) {
        ...
        throw new Error(`聊天完成失败: ${errorMessage} (状态码: ${response.status})`);
    }
        ...
        <div className="flex flex-row items-center justify-between">
            ...
            <div className="relative flex flex-row rounded-full hover:bg-gray-300">
                <div className="flex flex-row p-1 px-3 cursor-pointer" onClick={() => setShowRoleMenu(!showRoleMenu)}>
                    <LuUser className="mr-2" size={25} /> <span className="font-bold">{msg.role}</span>
                </div>
                {showRoleMenu && (
                    <>
                        ...
                        <div className="absolute bottom-full left-0 mb-1 p-2 bg-white border border-gray-300 rounded-lg z-20">
                            ...
                            <div className="cursor-pointer hover:bg-gray-200 p-2" onClick={() => { setMsg(prev => new TextMessage(SpecialRoles.SYSTEM, prev.content)); setShowRoleMenu(false); }} role="button" aria-label={t('system')}>{t('system')}</div>
                            <div className="cursor-pointer hover:bg-gray-200 p-2" onClick={() => { setMsg(prev => new TextMessage(SpecialRoles.ASSISTANT, prev.content)); setShowRoleMenu(false); }} role="button" aria-label={t('assistant')}>{t('assistant')}</div>
                            <div className="cursor-pointer hover:bg-gray-200 p-2" onClick={() => { setMsg(prev => new TextMessage(SpecialRoles.USER, prev.content)); setShowRoleMenu(false); }} role="button" aria-label={t('user')}>{t('user')}</div>
                        </div>
                    </>
                )}
            </div>
                    ...
                    <div className="mb-4">
                        {isEditing ? (
                            <textarea
                                ...
                                placeholder={t('Edit your message here')}
                            />
                        ) : (
                            <div className={`${blurText ? 'blur-sm' : ''}`}>
                                {showDiff ? (
                                    ...
                                    <div className="whitespace-pre-wrap break-words" aria-label={t('Edited message content')}>
                                        {editedText}
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="flex flex-row">
                            ...
                            {allowFollowUpDiscussion &&
                                <button
                                    ...
                                    aria-label={t('Follow-up discussions')}
                                >

...
export class SpeakAIMessage extends Message {
    quackCount: number
    static readonly type = 'speakAI'

    constructor(role: string, displayToUser: boolean = true, includedInChatCompletion: boolean = true, quackCount?: number) {
        super(SpeakAIMessage.type, role, displayToUser, includedInChatCompletion)
    ...
    static deserialize(serialized: string): SpeakAIMessage {
        const { role, displayToUser, includedInChatCompletion, quackCount } = JSON.parse(serialized);
        return new SpeakAIMessage(role, displayToUser, includedInChatCompletion, quackCount);
    }

    component() {
        return SpeakAIMessageComponent
    }
...
export const SpeakAIMessageComponent = ({ message: untypedMessage, messageID, updateMessage, className }: {
    message: Message; messageID: number; updateMessage: (messageID: number, message: Message) => void; className?: string;
}) => {
    const { t } = useTranslation()
    const message = untypedMessage as unknown as SpeakAIMessage
